...
// API: LLM Suggestion (OpenAI if key provided, else mock)
app.post('/api/llm/suggest', requireAuth, async (req, res) => {
  try {
    const { prompt, language, userCode, problemTitle, problemStatement } = req.body || {};
    ...
    if (!openAiKey && !openRouterKey) {
      ...
      const probKey = title.includes('two sum') ? 'two-sum'
      ...
      if (probKey === 'two-sum') {
        suggestions.push(
          'Use a hash map to store value->index and check complement target - x in one pass.',
          'Return indices as soon as you find a match to keep linear time.',
          'Watch out for using the same element twice; insert after checking complement.'
        );
      } else if (probKey === 'reverse-string') {
        suggestions.push(
          'Convert to a list/array of characters and reverse it, or use slicing in Python.',
          'Ensure you handle empty strings and single characters gracefully.',
          'Keep it O(n) time and O(n) additional space (or in-place if allowed).'
        );
      } else if (probKey === 'fizz-buzz') {
        suggestions.push(
          'Iterate from 1..n and build the output list of strings.',
          'Concatenate "Fizz" for multiples of 3 and "Buzz" for multiples of 5.',
          'If neither applies, append the number itself as a string.'
        );
      } else {
        suggestions.push(
          'Clarify input and output formats; define solve() signature clearly.',
          'Start with a straightforward solution, then optimize hot spots.',
          'Add tests for edge cases and verify against the sample cases.'
        );
      }

      // Code health hints based on user code
      if (!code || code.length < 20 || /TODO|pass\b|return null|return None/.test(code)) {
        suggestions.unshift('Start from a minimal working solve() and handle the simplest cases first.');
      }
      if (/while\s*\(true\)|for\s*\(;;\)/.test(code)) {
        suggestions.push('Avoid unbounded loops; ensure your loop has a clear termination to prevent TLE.');
      }

      return res.json({
        model: 'mock-suggester',
        suggestions,
        snippet: snippetFor(probKey),
      });
    }

    // Provider selection: OpenAI first, then OpenRouter
    const sys = 'You are an expert coding assistant. Provide concise, actionable hints and a short code snippet tailored to the user\'s problem and code. Prefer correctness and clarity.';
    const userMsg = `Problem: ${problemTitle || 'N/A'}\nLanguage: ${language || 'N/A'}\nStatement:\n${problemStatement || 'N/A'}\n\nUser prompt: ${prompt}\n\nCurrent code:\n${(userCode || '').slice(0, 4000)}\n\nPlease provide:\n- 3-5 specific recommendations (bulleted)\n- A short, self-contained snippet for the core idea`;
    try {
      if (openAiKey) {
      ...
      const suggestions = [
        'Clarify input/output and implement solve() matching the expected signature.',
        'Aim for linear-time solutions with hash maps or two pointers where applicable.',
        'Write a few small tests locally before submitting to the judge.'
      ];
